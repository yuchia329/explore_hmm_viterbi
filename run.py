...
def evaluate(test_sentences, tagged_test_sentences):
    ...
    pred = [str(tag)
            for sentence in tagged_test_sentences for token, tag in sentence]
...
def main():
    ...
    tagged_test_sentences = [nltk.pos_tag(
        [token for token, tag in sentence]) for sentence in test_sentences]
